...
function Steam() {
  ...
  const [positions, meta] = useMemo(() => {
    const pos = new Float32Array(count * 3);
    const m = Array.from({ length: count }, () => ({
      speed: 0.004 + Math.random() * 0.006,
      xOff: (Math.random() - 0.5) * 0.35,
      zOff: (Math.random() - 0.5) * 0.35,
      phase: Math.random() * Math.PI * 2,
...
function FallbackBox() {
  ...
  return (
    <mesh ref={mesh}>
      ...
      <meshStandardMaterial color="#5c4536" roughness={0.4} metalness={0.2} />
    </mesh>
  );
}
...
function Scene() {
  ...
  useEffect(() => {
    // Ensure pixel ratio is set correctly after context creation
...
export function Experience3D() {
  ...
  return (
    <section id="experience" className="relative overflow-hidden bg-espresso-950">
      <div className="section-padding relative z-10 mx-auto flex max-w-7xl flex-col gap-8 lg:flex-row lg:items-center">
        <div className="lg:w-2/5">
          <span className="text-[10px] tracking-[0.5em] text-gold-500/70 uppercase">Immersive</span>
          <h2 className="mt-4 font-display text-5xl leading-tight text-cream-100 md:text-6xl lg:text-7xl">
        ...
        <div ref={containerRef} className="relative flex-1" style={{ height: "clamp(360px, 55vw, 70vh)", minHeight: "360px" }}>
          {/* Border/glow layer — separate from canvas so backdrop-filter never wraps the WebGL context */}
          <div
          ...
          {/* Canvas container — plain overflow:hidden, NO backdrop-filter */}
          <div
            className="absolute inset-0 rounded-3xl"
            style={{ overflow: "hidden", borderRadius: "1.5rem", zIndex: 1 }}
          >
